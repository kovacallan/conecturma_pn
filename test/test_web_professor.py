...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        test_app.post('/observador/create_observador_professor', dict(nome='proof',senha="spam",telefone="21 99887342",email="teste@teste.test.te",tipo_observador="3",escola="1"))
        test_app.post('/login_observador', dict(usuario='proof', senha='spam'))

    def _fixprof(self):
        res =test_app.get("/create_medalha", dict(nome="medalha0",tipo="1"))
        res = test_app.post('/observador/create_observador_gestor',)
    ...
    def test_medalha_cadastro(self):
        ...
        self.assertEqual(res.status_int, 200)
        self.assertIn('<form action="create_medalha"> <br>',res.text,res.text)
